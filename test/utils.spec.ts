...
test.serial('allMarketCode()', async t => {
...
test.serial('tickers(): sort market name', async t => {
...
test.serial('tickers(): sort acc_trade_price_24h', async t => {
  const codes = await allMarketCode()
...
test.serial('tickers(): sort acc_trade_price_24h (splice)', async t => {
  const codes = await allMarketCode()
  const tks = await tickers(codes, 'acc_trade_price_24h')
  while(tks.length !== 0) {
...
// test('safeApi#getMarket()', async t => {
//   const ress = []
//   for(let i = 0; i < 200; i++) {
//     ress.push((async () => {
